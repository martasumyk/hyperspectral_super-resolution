...
def crop_s2_stack_overlap_keep_bandnames(
    s2_stack_path,
    out_path,
    left, bottom, right, top,
    overwrite=False,
    ...
    with rasterio.open(s2_stack_path) as src:
        src_desc = list(src.descriptions) 
        print("SRC descriptions:", src_desc)

        w = from_bounds(left, bottom, right, top, transform=src.transform)
        w_int = w.round_offsets().round_lengths()

        full = Window(0, 0, src.width, src.height)
        w_int = w_int.intersection(full)
        if w_int.width <= 0 or w_int.height <= 0:
            raise ValueError("Overlap window is empty after clipping. Check -te.")
        ...
        profile.update(
            ...
            width=int(w_int.width),
            height=int(w_int.height),
            transform=win_transform(w_int, src.transform),
            tiled=True,
            compress="DEFLATE",
            predictor=2,
            zlevel=1,
    ...
    # 2) force descriptions + redundant tag in a second pass
    with rasterio.open(out_path, "r+") as dst:
        for i in range(1, dst.count + 1):
            name = src_desc[i - 1] if i - 1 < len(src_desc) else None
            if name:
                dst.set_band_description(i, str(name))
                # redundant, in case descriptions get lost again
                dst.update_tags(i, BAND_NAME=str(name))

    if print_descriptions:
        with rasterio.open(out_path) as chk:
            print("OUT descriptions:", chk.descriptions)
            print("OUT band1 tags:", chk.tags(1))
